 ...
 * Software Whitelist Management Class
 * 
 * @author  Abner Liu
 * @license GPL-2.0+
 */
...
/**
 * PluginSoftwaremanagerSoftwareWhitelist class
 */
class PluginSoftwaremanagerSoftwareWhitelist extends CommonDBTM {
    ...
    /**
     * Add software to whitelist (static method)
     *
    ...
    static function addToList($software_name, $comment = '') {
        ...
        if (count($existing) > 0) {
            Session::addMessageAfterRedirect(__('Software already in whitelist', 'softwaremanager'), false, WARNING);
            return false;
        }

        // Remove from blacklist if exists
        $blacklist_table = PluginSoftwaremanagerSoftwareBlacklist::getTable();
        $DB->update($blacklist_table, 
            ['is_active' => 0], 
            ['name' => $software_name]
        );
        ...
        if ($result) {
            Session::addMessageAfterRedirect(__('Software added to whitelist successfully', 'softwaremanager'), true);
            return true;
        }
